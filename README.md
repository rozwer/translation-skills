...
> ## ⚠️ このリポジトリは public（誰でも閲覧・ダウンロード可）
> 公開しているのは**枠組み（スキル・仕様・テンプレート）**だけ。会社が育てる中身は手元に残す。
> - **クライアント原稿・実名・実名⇔トークン対応表は絶対に commit しない**（`.gitignore` で多重に防いでいるが、最後の砦は人の確認）。
> - `glossary/` の実用語ファイル・ルーター実ログ・下訳は追跡対象外（手元のみ）。
> - `improvement/learnings.md` は空の雛形を同梱。**会社固有のルールを蓄積し始め、それを公開したくない場合は、最初の運用前に `improvement/learnings.md` を `.gitignore` へ追加する**（または private fork に切り替える）。
> - push 前に必ず `git status` と差分を目視し、機密が混じっていないか確認すること。
